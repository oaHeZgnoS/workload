...
<body>
	<form action="workload/display-list2.action?term=${term}" class="query-form">
		<span>学期：</span>
		<select name="term">
	...
	<c:if test="${sessionScope.dbUser.authority==1}">
		<c:forEach begin="1" end="${pageCount}" step="1" varStatus="st">
			<a href="workload/display-list2.action?page=${st.index}&term=${workloadList.get(0).term}" <c:if test="${currentPage==st.index}">class="currentPage"</c:if>>${st.index}</a>
		</c:forEach>
	</c:if>
	页
